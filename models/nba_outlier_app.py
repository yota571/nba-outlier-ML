...
def load_prizepicks_nba_props() -> pd.DataFrame:
    ...
    try:
        pp = PrizePick()
        raw = pp.get_data(organize_data=False)
        st.write('DEBUG: Raw PrizePicks items:', len(raw) if isinstance(raw, list) else type(raw))
    except TypeError:
        pp = PrizePick()
        raw = pp.get_data(False)
    ...
    st.write('DEBUG: Starting to build records from raw PrizePicks data')
...
# SIDEBAR UI
# =====================================================
st.sidebar.title("NBA Outlier-Style App (PrizePicks) – v2.1.0 ML")
...
# DEBUG: inspect Bobby Portis lines in edges_df BEFORE override
try:
    st.write("DEBUG: Bobby Portis rows in edges_df (before override)")
    st.write(
        edges_df[
            edges_df["player"].astype(str).str.contains("Bobby Portis", case=False, na=False)
        ][["player", "team", "opponent", "market", "line"]]
    )
except Exception:
    pass

# DEBUG: inspect Bobby Portis lines in props_df (live PrizePicks data)
try:
    if isinstance(props_df, pd.DataFrame) and "player_name" in props_df.columns:
        st.write("DEBUG: Bobby Portis rows in props_df (live PrizePicks)")
        st.write(
            props_df[
                props_df["player_name"].astype(str).str.contains("Bobby Portis", case=False, na=False)
